...
#INICIA CREACION DE TABLAS PRINCIPALES 
...
select  v.nombre, v.apellido, v.direccion ,v.fecha_primera_sospecha,
v.fecha_confirmacion,v.fecha_muerte,v.estado,count(m.victima_ID) as total
from victima as v , victima_asociado as m ,asociado as a
where v.ID=m.victima_ID and v.hospital_ID=m.victima_hospital_ID and m.asociado_ID=a.ID
group by v.nombre, v.apellido, v.direccion ,v.fecha_primera_sospecha,
v.fecha_confirmacion,v.fecha_muerte,v.estado
...
select distinct * from Temporal as t 
where nombre_hospital="Samaritan Medical Clinic" and 
direccion_hospital="9434 Mraz Points"
group by 
nombre_victima,
apellido_victima,
direccion_victima,
...
select  v.hospital_ID,h.nombre,h.direccion,count(v.fecha_muerte),((count(v.fecha_muerte) /1002)*100) as Porcentaje
from hospital as h  
inner JOIN victima as v  ON h.ID=v.hospital_ID
#where  h.nombre!=""
group by v.hospital_ID,h.nombre,h.direccion;
